...
describe Caridina::Events::Member do
    ...
    event = Caridina::Events::Event.from_json(%(
  {
    ...
    if invite_room_state = event.unsigned.try &.invite_room_state
      ...
      event = invite_room_state[0].as(Caridina::Events::StrippedUnknown)
      event.sender.should eq("@bob:example.org")
      event.state_key.should eq("")
      event.type.should eq("m.room.name")

      event = invite_room_state[1].as(Caridina::Events::StrippedJoinRules)
      event.content.join_rule.should eq(Caridina::Events::JoinRules::JoinRule::Invite)
      event.sender.should eq("@bob:example.org")
      event.state_key.should eq("")
      event.type.should eq("m.room.join_rules")
    end
